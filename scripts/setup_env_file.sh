...
# Update BRAVE_API_KEY and GGUF_FILE_NAME from script arguments
BRAVE_API_KEY="${1:-}"
GGUF_FILE_NAME="${2:-}"

if [[ -z "$BRAVE_API_KEY" ]]; then
  echo "[ERROR] Missing required first argument: BRAVE_API_KEY" >&2
  echo "Usage: $0 <BRAVE_API_KEY> <GGUF_FILE_NAME>" >&2
  exit 1
fi

if [[ -z "$GGUF_FILE_NAME" ]]; then
  echo "[ERROR] Missing required second argument: GGUF_FILE_NAME" >&2
  echo "Usage: $0 <BRAVE_API_KEY> <GGUF_FILE_NAME>" >&2
  exit 1
fi

# Write/update them in .env
upsert_env_kv "BRAVE_API_KEY" "$BRAVE_API_KEY"
upsert_env_kv "GGUF_FILE_NAME" "$GGUF_FILE_NAME"

# Generate and persist DJANGO_SECRET_KEY if missing
EXISTING_DJANGO_SECRET="$(get_env_val DJANGO_SECRET_KEY)"

if [[ -z "$EXISTING_DJANGO_SECRET" ]]; then
  DJANGO_SECRET_KEY="$(python -c "import secrets; print(secrets.token_urlsafe(50))")"
  upsert_env_kv "DJANGO_SECRET_KEY" "$DJANGO_SECRET_KEY"
  log "Generated new DJANGO_SECRET_KEY"
else
  DJANGO_SECRET_KEY="$EXISTING_DJANGO_SECRET"
  log "Existing DJANGO_SECRET_KEY found; not modifying"
fi
...
# Update DJANGO_ALLOWED_HOSTS by appending PUB_IP
EXISTING_ALLOWED_HOSTS="$(get_env_val DJANGO_ALLOWED_HOSTS)"

if [[ -z "$EXISTING_ALLOWED_HOSTS" ]]; then
  NEW_ALLOWED="$PUB_IP"
else
  # append only if not already present
  if [[ "$EXISTING_ALLOWED_HOSTS" == *"$PUB_IP"* ]]; then
      NEW_ALLOWED="$EXISTING_ALLOWED_HOSTS"
  else
      NEW_ALLOWED="${EXISTING_ALLOWED_HOSTS},${PUB_IP}"
  fi
fi

upsert_env_kv "DJANGO_ALLOWED_HOSTS" "$NEW_ALLOWED"
log "Updated DJANGO_ALLOWED_HOSTS=$NEW_ALLOWED"
